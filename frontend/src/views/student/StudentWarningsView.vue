...
<template>
  <div class="space-y-6">
    <div class="grid gap-4 md:grid-cols-2">
      <Card>
        <CardHeader class="pb-2">
          <CardTitle class="text-sm font-medium text-[#64748b]">挂科门数</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="flex items-center justify-between">
            <p class="text-3xl font-bold text-[#dc2626]">{{ failCount }}</p>
            <div class="grid size-10 place-items-center rounded-md bg-[#fef2f2]">
              <AlertTriangle class="size-5 text-[#dc2626]" />
            </div>
          </div>
          <p class="mt-1 text-xs text-[#64748b]">门</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader class="pb-2">
          <CardTitle class="text-sm font-medium text-[#64748b]">当前最高预警级别</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="flex items-center justify-between">
            ...
            <div class="grid size-10 place-items-center rounded-md bg-[#fff7ed]">
              <Award class="size-5 text-[#ea580c]" />
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <div class="space-y-4">
      <h3 class="text-sm font-semibold text-[#0f172a]">预警详情</h3>
      ...
      <div v-else-if="warnings.length === 0" class="py-12 text-center">
        ...
        <p class="mt-2 text-sm text-[#64748b]">暂无预警信息</p>
      </div>
      <div
      ...
      >
        <div class="flex items-center justify-between border-b border-inherit px-4 py-3">
          <div class="flex items-center gap-3">
            <component :is="getTypeIcon(item.type)" class="size-4 text-[#64748b]" />
            <span class="text-sm font-semibold text-[#0f172a]">{{ item.type }}</span>
          </div>
        ...
        <div class="px-4 py-3">
          <div v-if="item.courseName" class="mb-2">
            <span class="text-xs text-[#64748b]">课程：</span>
            <span class="text-sm font-medium text-[#475569]">{{ item.courseName }}</span>
          </div>
          <div class="mb-2 flex items-center gap-4">
            <div>
              <span class="text-xs text-[#64748b]">当前值：</span>
              <span class="text-sm font-semibold text-[#0f172a]">{{ item.currentValue }}</span>
            </div>
            <div>
              <span class="text-xs text-[#64748b]">阈值：</span>
              <span class="text-sm font-semibold text-[#0f172a]">{{ item.threshold }}</span>
            </div>
          </div>
          <p class="text-sm text-[#475569]">{{ item.reason }}</p>
        </div>
      </div>
    </div>

    <div v-if="suggestions.length > 0" class="space-y-4">
      <h3 class="text-sm font-semibold text-[#0f172a]">改进建议</h3>
      <div class="grid gap-4 md:grid-cols-2">
        <div
        ...
        >
          <div class="mb-2 flex items-center gap-2">
            <component :is="getTypeIcon(item.type)" class="size-4 text-[#155e75]" />
            <span class="text-sm font-semibold text-[#0f172a]">{{ item.type }}</span>
          </div>
          <p class="text-sm text-[#475569]">{{ item.suggestion }}</p>
        </div>
      </div>
    </div>
  </div>
