...
<template>
  <div class="space-y-6">
    <div class="mb-6 flex items-center gap-4">
      ...
      <div class="flex items-center gap-2">
        <label class="text-sm font-medium text-[#475569] dark:text-gray-300">{{ t('score.examType') }}</label>
        <select
    ...
    <div v-if="profile" class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
      <div class="flex items-center gap-6">
        <div class="grid size-14 place-items-center rounded-full bg-[#ccfbf1] text-[#0f766e] dark:bg-teal-900/30 dark:text-teal-400">
          <User class="size-7" />
        </div>
        <div class="flex flex-1 items-center gap-8">
          <div>
            <p class="text-xs text-[#64748b] dark:text-gray-400">{{ t('student.name') }}</p>
            <p class="text-sm font-semibold text-[#0f172a] dark:text-white">{{ profile.name }}</p>
          </div>
          <div>
            <p class="text-xs text-[#64748b] dark:text-gray-400">{{ t('student.studentId') }}</p>
            <p class="text-sm font-semibold text-[#0f172a] dark:text-white">{{ profile.studentNo }}</p>
          </div>
          <div>
            <p class="text-xs text-[#64748b] dark:text-gray-400">{{ t('student.class') }}</p>
            <p class="text-sm font-semibold text-[#0f172a] dark:text-white">{{ profile.className }}</p>
          </div>
          <div>
            <p class="text-xs text-[#64748b] dark:text-gray-400">{{ t('student.major') }}</p>
            <p class="text-sm font-semibold text-[#0f172a] dark:text-white">{{ profile.major }}</p>
          </div>
          <div>
            <p class="text-xs text-[#64748b] dark:text-gray-400">{{ t('student.enrollmentYear') }}</p>
            <p class="text-sm font-semibold text-[#0f172a] dark:text-white">{{ profile.enrollmentYear }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      ...
      <div class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between">
          ...
          <div class="grid size-10 place-items-center rounded-md bg-[#e0f2fe] dark:bg-blue-900/30">
            <BarChart3 class="size-5 text-[#155e75] dark:text-blue-400" />
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between">
          ...
          <div class="grid size-10 place-items-center rounded-md bg-[#f0fdf4] dark:bg-green-900/30">
            <CreditCard class="size-5 text-[#15803d] dark:text-green-400" />
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-[#64748b] dark:text-gray-400">{{ t('ranking.classRanking') }}</p>
            <p class="mt-1 text-2xl font-semibold text-[#155e75] dark:text-blue-400">
              {{ stats?.classRank ?? 0 }}<span class="text-sm text-[#94a3b8] dark:text-gray-500">/{{ stats?.classTotal ?? 0 }}</span>
            </p>
          </div>
          <div class="grid size-10 place-items-center rounded-md bg-[#e0f2fe] dark:bg-blue-900/30">
            <Users class="size-5 text-[#155e75] dark:text-blue-400" />
          </div>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-[#64748b] dark:text-gray-400">{{ t('ranking.gradeRanking') }}</p>
            <p class="mt-1 text-2xl font-semibold text-[#b45309] dark:text-orange-400">
              {{ stats?.gradeRank ?? 0 }}<span class="text-sm text-[#94a3b8] dark:text-gray-500">/{{ stats?.gradeTotal ?? 0 }}</span>
            </p>
          </div>
          <div class="grid size-10 place-items-center rounded-md bg-[#fef3c7] dark:bg-orange-900/30">
            <Trophy class="size-5 text-[#b45309] dark:text-orange-400" />
          </div>
        </div>
      </div>
    </div>
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <div class="text-right">
                <p class="text-xs text-[#155e75] dark:text-blue-400">{{ t('student.class') }}</p>
                <p class="text-sm font-medium text-[#155e75] dark:text-blue-400">
                  {{ course.classRank }}<span class="text-[#94a3b8] dark:text-gray-500">/{{ course.classTotal }}</span>
                </p>
              </div>
              <div class="text-right">
                <p class="text-xs text-[#b45309] dark:text-orange-400">{{ t('class.grade') }}</p>
                <p class="text-sm font-medium text-[#b45309] dark:text-orange-400">
                  {{ course.gradeRank }}<span class="text-[#94a3b8] dark:text-gray-500">/{{ course.gradeTotal }}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="rounded-lg border border-[#e2e8f0] bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        ...
        <div v-else class="flex h-[500px] items-center justify-center">
          <div class="text-center">
            ...
            <p class="mt-2 text-sm text-[#64748b] dark:text-gray-400">{{ t('common.noData') }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="stats?.creditWarning" 
         class="rounded-lg border border-[#fed7aa] bg-[#fff7ed] p-4 dark:border-orange-700 dark:bg-orange-900/30">
      <div class="flex items-center gap-2">
        <AlertTriangle class="size-4 text-[#ea580c] dark:text-orange-400" />
        <h2 class="text-sm font-semibold text-[#9a3412] dark:text-orange-300">{{ t('warning.scoreWarning') }}</h2>
      </div>
      <p class="mt-2 text-sm text-[#9a3412] dark:text-orange-300">
        {{ t('overview.completedCredits') }}：<span class="font-semibold">{{ stats.earnedCredit }}</span> / 
